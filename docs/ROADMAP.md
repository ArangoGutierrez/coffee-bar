...
| **SECURITY.md** | Private disclosure route, supported versions, explicit scope statement: coffee-bar reads agent metadata only and never transcript contents (handoff §12), makes exactly one outbound request and says in full what that request carries (issue #29 — it was "makes no network egress" until the update check landed), never elevates its own privilege, and — now that M5 has landed — what the opt-in root path may do and what refusing a journal costs. |
