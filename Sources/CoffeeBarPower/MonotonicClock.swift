...
/// needs. `WatchdogService` compares the journal's `setAt` against
/// `kern.boottime` and reverts anything older than the boot, so no deadline
/// here ever has to survive one.
public enum SystemMonotonicClock {
    ...
    private static let secondsPerTick: Double = {
        ...
        guard mach_timebase_info(&info) == KERN_SUCCESS, info.denom != 0 else {
            ...
            // platform ships, following `SystemBootTime.current`, where an
            // unreadable `kern.boottime` returns a value that reverts
            // everything rather than one that checks nothing.
